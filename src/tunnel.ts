  ...
  switch (runtime) {
    ...
    default:
      throw new WhisperError(
        "egress: this runtime has no raw-socket API, so it cannot route traffic in-process. Use a Node, Deno, or Cloudflare Workers runtime, or route through a backend fetch-forward gateway.",
        { status: 501 },
      );
  }
}
