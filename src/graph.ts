...
export class WhisperGraph {
   ...
   *
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.identify([$v]) YIELD host, vendor_id, canonical_name, category, roles, host_class, band
  ...
  identify(v: string = "api.openai.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.identify([$v]) YIELD host, vendor_id, canonical_name, category, roles, host_class, band", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.assess([$v]) YIELD host, label, band, sub_labels, coverage, evidence
  ...
  assess(v: string = "8.8.8.8", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.assess([$v]) YIELD host, label, band, sub_labels, coverage, evidence", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.variants($v) YIELD variant, method, exists, confidence
  ...
  variants(v: string = "paypal.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.variants($v) YIELD variant, method, exists, confidence", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.walk($v) YIELD coverage, host, nearest_known_vendors, no_atlas_match, siblings
  ...
  walk(v: string = "cloudflare.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.walk($v) YIELD coverage, host, nearest_known_vendors, no_atlas_match, siblings", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.explain($v) YIELD indicator, score, level, explanation, sources
  ...
  explain(v: string = "paypal.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.explain($v) YIELD indicator, score, level, explanation, sources", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.psl.tldPlusOne($v) YIELD apex
  ...
  pslTldplusone(v: string = "www.foo.co.uk", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.psl.tldPlusOne($v) YIELD apex", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.psl.affiliation($v) YIELD found, suffix, submitterOrg, submitterLogin, evidenceKind, confidence
  ...
  pslAffiliation(v: string = "paypal.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.psl.affiliation($v) YIELD found, suffix, submitterOrg, submitterLogin, evidenceKind, confidence", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.origins($v) YIELD ip, confidence, methods, asn, asnName, kind
  ...
  origins(v: string = "cloudflare.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.origins($v) YIELD ip, confidence, methods, asn, asnName, kind", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.history($v)
  ...
  history(v: string = "paypal.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.history($v)", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.history.whois($v) YIELD queryTime, createDate, updateDate, expiryDate, registrar, registrant, country, nameServers
  ...
  historyWhois(v: string = "paypal.com", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.history.whois($v) YIELD queryTime, createDate, updateDate, expiryDate, registrar, registrant, country, nameServers", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.asSet($v) YIELD asSetName, memberAsn, sourceRir
   * Returns columns: asSetName, memberAsn, sourceRir.
   *
   * @see https://www.whisper.security/docs/whisper-graph/procedures
   */
  asset(v: string = "AS-CLOUDFLARE", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.asSet($v) YIELD asSetName, memberAsn, sourceRir", { v: v }, reqOpts);
  }
   ...
   * KEYLESS direct read: serves with NO key at all (rate-limited taste,
   * real answers); a key, when present, is sent as X-API-Key and lifts the
   * limit. Cypher: CALL whisper.lookupTorRelay($v) YIELD indicator, found, fingerprint, exitAddressCount, source, ingestedAt
  ...
  lookupTorRelay(v: string = "185.220.101.33", reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL whisper.lookupTorRelay($v) YIELD indicator, found, fingerprint, exitAddressCount, source, ingestedAt", { v: v }, reqOpts);
  }
  ...
  dbSchema(reqOpts?: RequestOptions): Promise<GraphResult> {
    return this.runDirectKeyless("CALL db.schema()", {}, reqOpts);
  }
 ...
 */
const _RECIPES: readonly Recipe[] = [
  {
    "method": "anycastDnsRootSovereignty",
    "keyless": false,
    "mode": "flow",
    "summary": "Assess how resilient a country's core DNS is if it were cut off from the world.",
    "params": [
      "country"
    ],
  ...
  },
  {
    ...
    "keyless": false,
    "mode": "flow",
    "summary": "Find the choke points an attacker would target - and how any two things connect.",
    "params": [
      "value",
      "other"
    ],
  ...
  {
    "method": "attackSurface",
    "keyless": false,
    "mode": "flow",
    "summary": "Map everything about a domain that's exposed to the outside world, scored for risk.",
    "params": [
      "domain"
    ],
  ...
  {
    "method": "bgpHijackExposure",
    "keyless": false,
    "mode": "flow",
    "summary": "Grade a network's routing security and trace conflicts to the domains they'd expose.",
    "params": [
      "value"
    ],
  ...
  },
  {
    ...
    "keyless": false,
    "mode": "flow",
    "summary": "Pick one asset and see what would break if it failed - and what it depends on in turn.",
    "params": [
      "indicator"
    ],
  ...
  {
    "method": "buildTakedownEvidencePackage",
    "keyless": false,
    "mode": "flow",
    "summary": "Assemble a ready-to-submit dossier for taking down a scam or phishing domain.",
    "params": [
      "domain"
    ],
  ...
  {
    "method": "discoverAiAgentInfrastructure",
    "keyless": false,
    "mode": "flow",
    "summary": "Map an organisation's externally visible AI and agent endpoints from the outside.",
    "params": [
      "value"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/pentest-recon"
  },
  {
    ...
    "keyless": false,
    "mode": "flow",
    "summary": "Investigate one suspicious domain, IP, or network in depth and get a clear picture of the threat and everything connected to it.",
    "params": [
      "indicator"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/soc"
  ...
  {
    "method": "indicatorEnrichment",
    "keyless": false,
    "mode": "flow",
    "summary": "Turn one domain or IP into a full context card - owner, hosting, mail, location, reputation at a glance.",
    "params": [
      "value"
    ],
  ...
  {
    "method": "infrastructureMapping",
    "keyless": false,
    "mode": "flow",
    "summary": "Trace one indicator to its true owner and full estate, even behind privacy screens and CDNs.",
    "params": [
      "value"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/compliance"
  ...
  {
    "method": "mapSupplyChainConcentration",
    "keyless": false,
    "mode": "flow",
    "summary": "Grade an organisation for over-reliance on single providers, regions, or facilities.",
    "params": [
      "domain"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/compliance"
  ...
  {
    "method": "nameserverHijackDnsConsistency",
    "keyless": false,
    "mode": "flow",
    "summary": "Check a domain's name servers for the misconfigurations that enable DNS hijacking.",
    "params": [
      "value"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/dns-email"
  },
  {
    ...
    "keyless": false,
    "mode": "flow",
    "summary": "Profile a network or address block into a full routing and reachability health card.",
    "params": [
      "target"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/bgp-routing"
  ...
  {
    "method": "subdomainTakeover",
    "keyless": false,
    "mode": "flow",
    "summary": "Find subdomains that point at abandoned services an attacker could claim.",
    "params": [
      "value"
    ],
    "docsUrl": "https://www.whisper.security/docs/recipes/pentest-recon"
  ...
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Name the vendor and operator role behind a host or IP in one call.",
    "params": [
      "v"
    ],
  ...
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Get a labelled threat posture for a host or IP - malicious, benign, or unknown.",
    "params": [
      "v"
    ],
  ...
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Generate look-alike domain variants of a brand and see which are registered.",
    "params": [
      "v"
    ],
  ...
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Walk the graph to the nearest known vendors behind a host, with the channel and confidence.",
    "params": [
      "v"
    ],
    "docsUrl": "https://www.whisper.security/docs/whisper-graph/procedures"
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Score an indicator against the threat feeds and explain exactly why.",
    "params": [
      "v"
    ],
  ...
  {
    "method": "pslTldplusone",
    "keyless": true,
    "mode": "direct",
    "summary": "Reduce any hostname to its registrable apex (eTLD+1) via the Public Suffix List.",
    "params": [
      "v"
    ],
  ...
  {
    "method": "pslAffiliation",
    "keyless": true,
    "mode": "direct",
    "summary": "Check whether a domain is a PSL private-section suffix and who submitted it.",
    "params": [
      "v"
    ],
    "docsUrl": "https://www.whisper.security/docs/whisper-graph/procedures/helpers"
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Find the real origin IPs behind a CDN-fronted domain, ranked by confidence.",
    "params": [
      "v"
    ],
  ...
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Get the full historical WHOIS timeline for a domain.",
    "params": [
      "v"
    ],
  ...
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "Get the WHOIS-only historical timeline for a domain.",
    "params": [
      "v"
    ],
    "docsUrl": "https://www.whisper.security/docs/whisper-graph/procedures/history"
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "List the member ASNs of an AS-SET macro.",
    "params": [
      "v"
    ],
    "docsUrl": "https://www.whisper.security/docs/whisper-graph/procedures"
  ...
  {
    "method": "lookupTorRelay",
    "keyless": true,
    "mode": "direct",
    "summary": "Check whether an IP is a known Tor exit relay.",
    "params": [
      "v"
    ],
    "docsUrl": "https://www.whisper.security/docs/whisper-graph/procedures/helpers"
  },
  {
    ...
    "keyless": true,
    "mode": "direct",
    "summary": "List every node and relationship type in the graph with counts and examples.",
    "params": [],
  ...
  },
  {
    ...
    "keyless": false,
    "mode": "direct",
    "summary": "Contribute an indicator observation or feedback back into the graph (requires an API key).",
    "params": [
      "kind",
      "identifier_kind",
      "value",
